    ...
    (function( $ ) {
      ...
      $.fn.vitrinaboxTooltip = function (options) {     
        ...
        this.each(function(){
          ...
          //The default values for the tooltip's box and point
          if(options){
            if(options.count <= 0){
              options.count = 1
            }
          ...
          //Exception to the tag for hyperlink
          if(this.tagName === 'A'){
            boxWidth = width * 0.5;
            boxHeight = boxWidth / 3;
            defaultDiameter = 0;
            defaultThickness = 0;
            defaultFontSize = boxHeight / 1.6 + 'px';
            if(options){
              if(options.count >= 1){
                options.count = 1
              }
          ...
          let point_box_offset = boxHeight / 2;
        ...
        //Create and bind even listener for window scroll.
        $(window).bind('resize', this, changeBoxOnScroll);

        function changeBoxOnScroll(){
        ...
        function getBoxRelPos(currentTooltip, viewport, pointDiameter, pointPos){
          ...
          if(point.width() === 0){
            return chooseBoxRelativePosition('up', point);
          }
        ...
        function chooseBoxRelativePosition(relPos, point){
          ...
          let LeftAndRightY = pointTop +  wholePointDiameter / 2 - settings.boxHeight / 2 
        ...
        function getBoxInitialPosition(point){
          //Notification logic goes here later.
